...
struct SearchView: View {
    ...
    private func filterEvents(query: String) {
        ...
        filteredEvents = allEvents.compactMap { event -> Event? in
            // First check if it matches the search query
            let matches = event.name.lowercased().contains(lowercaseQuery) ||
                         event.description.lowercased().contains(lowercaseQuery) ||
                         event.location.lowercased().contains(lowercaseQuery)

            guard matches else { return nil }

            // Then deduplicate by ID + actual occurrence time
            let key = "\(event.id)_\(Int(event.time.timeIntervalSince1970))"
    ...
    private func preloadAllEvents() async {
        ...
        // Create events with actual occurrence dates from the date keys
        var eventsWithCorrectDates: [Event] = []
        var seenKeys = Set<String>()

        for (dateKey, events) in allByDate {
            for event in events {
                let correctedEvent = Event(
                    id: "\(event.id)_\(Int(dateKey.timeIntervalSince1970))", // Make ID unique per occurrence
                    name: event.name,
                    time: dateKey, // Use the actual occurrence date
                    endTime: Calendar.current.date(byAdding: .hour, value: 1, to: dateKey) ?? event.endTime,
                ...
                // Deduplicate by event ID + occurrence date
                let key = "\(event.id)_\(Int(dateKey.timeIntervalSince1970))"
                if !seenKeys.contains(key) {
                    seenKeys.insert(key)
                    eventsWithCorrectDates.append(correctedEvent)
                }
            }
        }

        let uniqueEvents = eventsWithCorrectDates

        await MainActor.run {
            allEvents = uniqueEvents.sorted { $0.time > $1.time }
        }
    }
}
