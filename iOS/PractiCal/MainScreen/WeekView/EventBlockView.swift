...
struct EventBlockView: View {
    ...
    private var columnWidth: CGFloat { 
        ...
        // Show debugging toast for width calculations AND positioning
        if isDebugMode, let toastManager = toastManager {
            ...
            let message = "\(dayName) '\(event.name)' @\(timeStr): startH=\(startHour), startM=\(startMinute), offset=\(Int(startOffset)), totCol=\(totalColumns)"
