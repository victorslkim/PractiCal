...
SOURCE_FILES=(
    ...
    "PractiCal/Shared/AppSettingsManager.swift"
    ...
    "PractiCal/MainScreen/Shared/CalendarConstants.swift"
    "PractiCal/MainScreen/Shared/TimeLabel.swift"
    "PractiCal/MainScreen/Shared/TimeGrid.swift"
    "PractiCal/MainScreen/Shared/WeekDateHeader.swift"
    "PractiCal/MainScreen/Shared/AllDayLabel.swift"
    "PractiCal/MainScreen/WeekView/WeekView.swift"
    "PractiCal/MainScreen/WeekView/AllDayEventsSection.swift"
    "PractiCal/MainScreen/WeekView/EventBlockView.swift"
    "PractiCal/MainScreen/DayView/DayView.swift"
    "PractiCal/MainScreen/DayView/DayEventBlockView.swift"
...
build_for_device() {
    ...
    if [[ -n "$TARGET_DEVICE_ID" ]]; then
        ...
        DEVICE_ID=$(xcrun devicectl list devices 2>/dev/null | grep -E "(connected|available \(paired\))" | head -1 | grep -o '[0-9A-F]\{8\}-[0-9A-F]\{4\}-[0-9A-F]\{4\}-[0-9A-F]\{4\}-[0-9A-F]\{12\}')
        if [[ -n "$DEVICE_ID" ]]; then
            local device_name=$(xcrun devicectl list devices 2>/dev/null | grep -E "(connected|available \(paired\))" | head -1 | awk -F'   ' '{print $1}')
